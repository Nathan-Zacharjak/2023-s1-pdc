// This program has been left alone with the exception of
// the line of code below, simply to prevent the actual
// output of the program from printing since we just want
// the timing output
// As well, line 152 has been modified to make the 2nd phase
// of calculating the forces done using cyclic partitioning
// rather than block partitioning, so that all calculations
// of the forces are using cyclic partitioning to see the
...
// #        pragma omp for
         // Making the 2nd phase of the forces calculation scheduled
         // cyclicly as well. The line commented out above has
         // been replaced with the line below
#        pragma omp for schedule(static,1)
