// Program modified by Nathan Zacharjak a1704695
...
int main(void) {
   ...
   // Vars to store each core's start, finish, and elapsed time
   double start_time, finish_time, elapsed_time;
   double core_times[comm_sz];
   ...
   // Start time measurements here
   // We're no longer dealing with setting up the histogram
   // we're actually calculating it at this point

   // First set up a barrier for each core
   MPI_Barrier(comm);

   // Then get a standard starting time for each of the cores
   ...
   // Let the code we're interested in timing run
   ...
   // Get the finishing time for each core and calculate the total time spent
   finish_time = MPI_Wtime();
   elapsed_time = finish_time - start_time;   
   
   // End time measurements here
   // From here onwards we're not performing any more
   // parallel histogram computation

   // Comment out printing out the histogram as that is only
   // done by 1 core, we're only interested in how cores
   // perform in parallel work
   ...
   free(data);
   free(local_data);
   // Now we need to find the maximum time between all the cores
   // This number is what we will use as the time measurement of the program's runtime

   // We need to do this after the histogram output prints so
   // the number ends up at the bottom of the output which
   // makes it easy to read
   MPI_Gather(&elapsed_time, 1, MPI_DOUBLE, core_times, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   if (my_rank == 0){
      double max = 0;
